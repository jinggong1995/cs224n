...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.target_vocab_projection = nn.Linear(
            in_features=self.hidden_size, out_features=len(vocab.tgt), bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        new_t = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        init_decoder_hidden = self.h_projection(new_t)
        
        new_c = torch.cat((last_cell[0], last_cell[1]), dim=1)
        init_decoder_cell = self.c_projection(new_c)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_splitted = torch.split(Y, 1, dim=0)
        for i in range(Y.shape[0]):
            Y_t = torch.squeeze(Y_splitted[i], 0)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        temp = torch.unsqueeze(dec_hidden, 2).permute(0, 2, 1)
        e_t = torch.squeeze(torch.bmm(temp,enc_hiddens_proj.permute(0, 2, 1)), 1)
